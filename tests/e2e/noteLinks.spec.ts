...
test.describe("ノート間リンク機能", () => {
  ...
  test("シナリオ3: バックリンクパネルに参照元ノートが表示される", async ({
    page,
  }) => {
    // 1. リンク先ノートを作成
  ...
  test("シナリオ4: 関連ノートウィジェットにスコア付きで表示される", async ({
    page,
  }) => {
    // 1. 共通タグを持つノートを作成
    ...
    // 4. 関連ノートBが表示されることを確認
    const relatedWidget = page.locator(
      '.related-notes, [data-component="related-notes"]',
    );
  ...
  test("シナリオ5: 存在しないノートへのリンクは赤色で表示される（赤リンク）", async ({
    page,
  }) => {
    // 1. 新規ノートを作成
    ...
    const brokenLink = page
      .locator(
        'a:has-text("存在しないノート"), span:has-text("存在しないノート")',
      )
      .first();
    await expect(brokenLink).toBeVisible();

    // 4. 赤色のスタイルを確認
    const linkColor = await brokenLink.evaluate(
      (el) => window.getComputedStyle(el).color,
    );

    // 赤系の色を期待（RGB値で確認）
    const isReddish =
      linkColor.includes("220, 38, 38") || // Tailwind red-600
  ...
  test("追加シナリオ: リンク削除後にバックリンクが更新される", async ({
    page,
  }) => {
    // 1. ノートを作成してリンクを追加
    ...
    await expect(
      page.locator("text=このノートへのリンクはありません"),
    ).toBeVisible();
  });
