...
async function runTests() {
  ...
  const html3 = '<html><head><title>Page Title</title></head><body><p>Content</p></body></html>';
  const title3 = extractTitle(html3);
  console.assert(title3 === 'Page Title', `Expected 'Page Title', got '${title3}'`);
  console.log('✓ Pass\n');

  // Test 4: extractMetadata
  console.log('Test 4: extractMetadata extracts createdAt');
  const html4 = '<html><head><meta name="created" content="2024-01-15T10:30:00Z"></head><body></body></html>';
  const metadata4 = extractMetadata(html4);
  console.assert(metadata4.createdAt instanceof Date, 'createdAt should be a Date');
  console.log('✓ Pass\n');
  ...
  console.assert(typeof result5.content === 'object', 'Content should be an object');
  console.assert(result5.metadata.createdAt instanceof Date, 'Metadata should have createdAt');
