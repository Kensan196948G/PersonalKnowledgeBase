...
describe("EmbeddingService", () => {
  ...
  describe("generateEmbedding", () => {
    ...
    it("should throw error when note not found", async () => {
      ...
      await expect(
        service.generateEmbedding("invalid-id"),
    ...
    beforeEach(() => {
      mockPrisma.note.findUnique.mockImplementation(({ where }) => {
        return Promise.resolve(
          mockNotes.find((n) => n.id === where.id) || null,
        );
      });
  ...
  describe("verifyModel", () => {
    ...
    it("should check default model when not specified", async () => {
      mockClient.listModels.mockResolvedValue([OLLAMA_MODELS.EMBEDDINGS]);
    ...
    it("should return false on API error", async () => {
      mockClient.listModels.mockRejectedValue(new Error("Connection error"));
