...
export function tiptapJsonToHtml(jsonString: string): string {
  console.log("[tiptapJsonToHtml] Called with string length:", jsonString?.length);
  console.log("[tiptapJsonToHtml] Input preview:", jsonString?.substring(0, 100));
  ...
  try {
    ...
    console.log("[tiptapJsonToHtml] Parsed JSON:", JSON.stringify(json).substring(0, 200));
    ...
    console.error("[tiptapJsonToHtml] Failed to convert TipTap JSON to HTML:", error);
