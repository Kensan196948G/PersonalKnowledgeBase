...
export function OneNoteImportModal({
  isOpen,
  onClose,
  onSuccess,
}: OneNoteImportModalProps) {
  const [file, setFile] = useState<File | null>(null);
  const [importing, setImporting] = useState(false);
  const [addImportTag, setAddImportTag] = useState(true);
  const [importType, setImportType] = useState<
    "html" | "docx" | "pdf" | "onepkg" | "mht"
  >("html");
  ...
  const handleTypeChange = (
    newType: "html" | "docx" | "pdf" | "onepkg" | "mht",
  ) => {
        ...
        >
          ...
          <div className="px-6 py-4 space-y-4">
            ...
            <div className="p-3 bg-blue-50 rounded text-sm text-blue-800">
              ...
              <p className="text-xs">
                {importType === "html" &&
                  "OneNoteで「ファイル」→「エクスポート」→「Webページ (.html)」を選択してエクスポートしたファイルをアップロードしてください。"}
                {importType === "docx" &&
                  "OneNoteで「ファイル」→「エクスポート」→「Word文書 (.docx)」を選択してエクスポートしたファイルをアップロードしてください。"}
                {importType === "pdf" &&
                  "OneNoteで「ファイル」→「エクスポート」→「PDF (.pdf)」を選択してエクスポートしたファイルをアップロードしてください。"}
                {importType === "onepkg" &&
                  "OneNoteで「ファイル」→「エクスポート」→「OneNoteパッケージ (.onepkg)」を選択してエクスポートしたファイルをアップロードしてください。"}
                {importType === "mht" &&
                  "OneNoteで「ファイル」→「エクスポート」→「単一ファイルWebページ (.mht/.mhtml)」を選択してエクスポートしたファイルをアップロードしてください。"}
              </p>
            </div>
          </div>
